    ...
    def __init__(self, model, options):
        ...
        """if self.sample == 'sliding_window':
            train_logs, train_labels = sliding_window(self.data_dir,
                                                  datatype='train',
                                                  window_size=self.window_size , num_classes=self.num_classes , padding_option=self.padding_option)
            val_logs, val_labels = sliding_window(self.data_dir,
                                              datatype='val',
                                              window_size=self.window_size,
                                              num_classes=self.num_classes, padding_option=self.padding_option,sample_ratio=0.001,)

        else:
            raise NotImplementedError"""
        
        train_dataset = log_dataset(logs=train_result_logs,
                                    labels=train_labels,
                                    seq=self.sequentials)
        valid_dataset = log_dataset(logs=test_result_logs,
                                    labels=test_labels,
                                    seq=self.sequentials)
        ...
        del train_result_logs, train_labels, train_groups, test_result_logs, test_labels, test_groups
        gc.collect()
